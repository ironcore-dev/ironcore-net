...
func (r *NetworkPeeringReconciler) reconcile(ctx context.Context, log logr.Logger, network *networkingv1alpha1.Network) (ctrl.Result, error) {
	...
	for _, peering := range network.Spec.Peerings {
		peeringClaimRef, peeringName, peeredID, err := r.reconcilePeering(ctx, log, network, peering)
		...
		if peeringName != "" {
			peeringNames = append(peeringNames, peeringName)
		}
		if peeredID != "" {
			peeredIDs = append(peeredIDs, peeredID)
		}
	}

	if len(peeringClaimRefs) > 0 {
		log.V(1).Info("Peering claim refs require network spec update")
		if err := r.updateSpec(ctx, log, network, peeringClaimRefs, peeredIDs); err != nil {
...
func (r *NetworkPeeringReconciler) updateSpec(
	...
	if err := r.updateApinetNetworkSpec(ctx, log, network, peeredIDs); err != nil {
		return fmt.Errorf("error updating apinet network spec: %w", err)
	}
...
func (r *NetworkPeeringReconciler) updateApinetNetworkSpec(
	ctx context.Context,
	log logr.Logger,
	network *networkingv1alpha1.Network,
	peeredIDs []string,
	...
	if err := r.APINetClient.Get(ctx, apinetNetworkKey, apinetNetwork); err != nil {
		return fmt.Errorf("apinet network is not created yet for network: %s \n %w", network.Name, err)
	}

	base := apinetNetwork.DeepCopy()
	apinetNetwork.Spec.PeeredIDs = peeredIDs
	log.V(1).Info("Updating apinet network spec peered ids")
	if err := r.APINetClient.Patch(ctx, apinetNetwork, client.StrategicMergeFrom(base)); err != nil {
		return fmt.Errorf("error updating apinet network spec peered ids: %w", err)
	}

	log.V(1).Info("Updated apinet network spec peered ids")
	return nil
}
...
func (r *NetworkPeeringReconciler) reconcilePeering(
...
) (networkingv1alpha1.NetworkPeeringClaimRef, string, string, error) {
	...
	for _, targetPeering := range targetNetwork.Spec.Peerings {
		...
		apinetNetwork := &apinetv1alpha1.Network{}
		if err := r.APINetClient.Get(ctx, client.ObjectKey{Namespace: r.APINetNamespace, Name: string(targetNetwork.UID)},
			apinetNetwork); err != nil {
			log.V(1).Info("apinet network for target network is not created yet")
			return networkingv1alpha1.NetworkPeeringClaimRef{}, "", "", nil
		}
		return peeringClaimRef, peering.Name, apinetNetwork.Spec.ID, nil
	}
...
func (r *NetworkPeeringReconciler) SetupWithManager(mgr ctrl.Manager, apiNetCache cache.Cache) error {
	log := ctrl.Log.WithName("networkpeering").WithName("setup")
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(
			...
			builder.WithPredicates(
				...
				r.networkStateAvailablePredicate(),
			),
		).
		Watches(
			&networkingv1alpha1.Network{},
