...
func validateNetworkPolicySpec(spec *core.NetworkPolicySpec, fldPath *field.Path) field.ErrorList {
	...
	if spec.Priority != nil && *spec.Priority < 0 {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("priority"), spec.Priority, "priority cannot be negative"))
	}
...
func validateNetworkPolicyPort(port *core.NetworkPolicyPort, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	...
	if protocol := port.Protocol; protocol != nil {
		allErrs = append(allErrs, ValidateProtocol(*protocol, fldPath.Child("protocol"))...)
	}

	return allErrs
}

func validateNetworkPolicyPeer(peer *core.NetworkPolicyPeer, supportedObjectSelectorKinds sets.Set[string], fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
...
func validateNetworkPolicyPeerObjectSelector(sel *core.ObjectSelector, allowedKinds sets.Set[string], fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, ValidateEnum(allowedKinds, sel.Kind, fldPath.Child("kind"), "must specify kind")...)
...
var supportedPolicyTypes = sets.New(
	core.PolicyTypeIngress,
	core.PolicyTypeEgress,
)

func validatePolicyType(policyType core.PolicyType, fldPath *field.Path) field.ErrorList {
	return ValidateEnum(supportedPolicyTypes, policyType, fldPath, "must specify type")
}

func validatePolicyTypes(policyTypes []core.PolicyType, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList

	seen := sets.New[core.PolicyType]()
...
func validateNetworkPolicySpecUpdate(newSpec, oldSpec *core.NetworkPolicySpec, fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList

	allErrs = append(allErrs, ValidateImmutableField(newSpec.NetworkRef, oldSpec.NetworkRef, fldPath.Child("networkRef"))...)
