...
func NewGetter(name string) (*config.Getter, error) {
	return config.NewGetter(config.GetterOptions{
		Name:       "metalnetlet",
		SignerName: certificatesv1.KubeAPIServerClientSignerName,
		Template: &x509.CertificateRequest{
			Subject: pkix.Name{
				CommonName:   onmetalapinetv1alpha1.MetalnetletCommonName(name),
				Organization: []string{onmetalapinetv1alpha1.MetalnetletsGroup},
			},
		},
...
func NewGetterOrDie(name string) *config.Getter {
	getter, err := NewGetter(name)
	if err != nil {
