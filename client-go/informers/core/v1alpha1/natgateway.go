...
func NewNATGatewayInformer(client onmetalapinet.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredNATGatewayInformer(client onmetalapinet.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.CoreV1alpha1().NATGateways(namespace).List(context.TODO(), options)
			},
...
func (f *nATGatewayInformer) defaultInformer(client onmetalapinet.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
