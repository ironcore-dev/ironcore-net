...
type NATTableLister interface {
	...
	List(selector labels.Selector) (ret []*corev1alpha1.NATTable, err error)
...
type nATTableLister struct {
	listers.ResourceIndexer[*corev1alpha1.NATTable]
}

// NewNATTableLister returns a new NATTableLister.
func NewNATTableLister(indexer cache.Indexer) NATTableLister {
	return &nATTableLister{listers.New[*corev1alpha1.NATTable](indexer, corev1alpha1.Resource("nattable"))}
}

// NATTables returns an object that can list and get NATTables.
func (s *nATTableLister) NATTables(namespace string) NATTableNamespaceLister {
	return nATTableNamespaceLister{listers.NewNamespaced[*corev1alpha1.NATTable](s.ResourceIndexer, namespace)}
}
